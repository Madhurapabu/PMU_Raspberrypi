        ...
        async with websockets.connect(uri) as websocket:
            ...
            while True:
                ...
                while True:
                    ...
                    for i in range(numSamples):
                        try:
                            ...
                            if line.startswith("Ph1:"):
                                ...
                                cos_raw = math.cos(
                                    2*math.pi*50*utc_timestamp_ph1)
                            ...
                            if (i == numSamples/2):
                                ...
                                utc_string = utc_datetime.strftime(
                                    "%Y-%m-%dT%H:%M:%SZ")

                            if count_2 == 1 and count_3 == 1:
                                f_time_3 = utc_timestamp_cos - utc_timestamp_ph1
                                count_2 = 0
                                count_3 = 0
                            # if count_3 == 2:
                            #     dt = datetime.datetime.now(timezone.utc)
                    ...
                    scale_mag_ph1 = -3.7719 + \
                        (1.3029*highPassFilteredMagnitude_ph1)

                    scale_mag_ph2 = -3.7719 + \
                        (1.2539*highPassFilteredMagnitude_ph2)
                    # o = (-0.009920634920635088*highPassFilteredMagnitude_ph2**2) + (4.454365079365151*highPassFilteredMagnitude_ph2)  -258.0877976190549
                    # y=-3.7719+1.2649x

                    scale_mag_ph3 = -3.7719 + \
                        (1.2139*highPassFilteredMagnitude_ph3)
                    ...
                    if sumReal_ph1 < 0:
                        if sumImag_ph1 < 0:
                            phase_ph1 = math.atan(
                                sumImag_ph1/sumReal_ph1) - math.pi
                        else:
                            phase_ph1 = math.atan(
                                sumImag_ph1/sumReal_ph1) + math.pi
                    elif sumImag_ph1 == 0:
                    ...
                    highPassFilteredMagnitude_an = math.degrees(
                        highPassFilteredMagnitude_an)
                    combined_message = f"v,{scale_mag_ph1},{scale_mag_ph2},{scale_mag_ph3},{f_time_3},{frequency_2},{frequency_3},{adjusted_time}"
                    # await websocket.send(str(scale_mag),str(frequency))  # Convert count to a string
                    # print(combined_message)
                    print("Scaled Freq", f_time_3)
